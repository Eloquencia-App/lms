...
if (isset($_POST['submit'])) {
    $req = $db->prepare('SELECT COUNT(*), ID FROM members WHERE email = :email AND password = :password');
    $req->execute(array(
    ...
    if ($requser[0] == 1) {
        ...
        $req = $db->prepare('INSERT INTO tokens(token, user_id, expiration) VALUES (:token, :id, :expiration)');
        $req->execute(array(
                'token' => $token,
                'id' => $requser['ID'],
...
if (isset($_POST['forgotSubmit'])) {
    if($_POST['captcha'] == $_SESSION['captcha']) {
        $req = $db->prepare("SELECT COUNT(*) FROM members WHERE email = :email");
        $req->execute(array(
            'email' => htmlspecialchars($_POST['email'])
        ));
        $req = $req->fetch();
        if ($req[0] == 1) {
            $token = rand(1000000, 9999999);
            $req = $db->prepare('UPDATE members SET reset = :token WHERE email = :email');
            $req->execute(array(
                'token' => $token,
                'email' => htmlspecialchars($_POST['email'])
            ));
    ...
    <div class="">
        <div class="card">
            <div class="card-header text-center align-items-center">
                <img src="assets/eloquencia_round.png" alt="Logo Eloquéncia" class="img-fluid" width="50">
